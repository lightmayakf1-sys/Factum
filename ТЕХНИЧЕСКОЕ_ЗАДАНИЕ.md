...
## Приложение "Normocontrol" — Анализ паспортов промышленного оборудования
...
**Normocontrol** — настольное GUI-приложение для автоматизированного извлечения технических параметров из документации на промышленное оборудование (паспорта, руководства, чертежи, каталоги, datasheet'ы) и формирования структурированных DOCX-карточек.
...
├── Normocontrol.bat             # Запуск двойным кликом
...
Файл конфигурации: `~/.normocontrol/config.json`
...
**Логика:** Если два значения из одного файла и их страницы близки (разница ≤ overlap) — это повторное извлечение с перекрывающихся страниц, а не настоящий конфликт. Оставляется первое (из более раннего чанка).
...
**Алгоритм:**
1. Сортировка по ключу `(SOURCE_PRIORITY.get(doc_type, 99), confidence_order.get(confidence, 99))`
2. `confidence_order = {"high": 0, "medium": 1, "low": 2}`
3. Берётся первый (наилучший) элемент
...
Обрабатывает 5 категорий из ответа Gemini:
...
5. Размеры: формат Д x Ш x В через " x " с пробелами. Единица в конце: "3 429 x 1 890 x 2 010 мм".
...
9. Если есть режимы работы (пуск/работа/промывка) -- перечисли через ";": "Пуск: 2,5 м3/ч; Работа: 1,8 м3/ч".
10. Перевод терминов: "[Переведённый термин] (Original Term)".
11. КОНТЕКСТ ИЗВЛЕЧЕНИЯ: Группы B--H описывают ТРЕБОВАНИЯ К ПЛОЩАДКЕ/ЗДАНИЮ
...
B.3. Масса нетто и масса с жидкостями (кг)
...
D.1. Установленная мощность P_уст (кВт) и потребляемая P_потр (кВт)
D.2. Напряжение (В), фазность (3ф/1ф), частота (Гц), ток (А)
D.3. Категория надёжности электроснабжения (I, II, III), ИБП
D.4. Тип пуска, cos phi, Ки, компенсация реактивной мощности
...
E.2. Расход (м3/ч или н.л/мин) -- средний и максимальный
E.3. Качество среды: класс чистоты, масло, точка росы
E.4. Точка подключения: диаметр, тип резьбы/фланца, координаты
...
F.4. Точка подключения: диаметр, тип соединения, координаты
F.5. Канализация: расход стоков, температура, состав
F.6. Точка слива: самотёк/давление, высота, диаметр
F.7. СОЖ: объём системы, марка, сепарация
F.8. Периодичность потребления

### G. Вентиляция, экология и шум (ОВ)
G.1. Локальные отсосы: диаметр патрубков, объём, разрежение
...
H.4. Микроклимат: температура (+/-C), влажность
...
Формат ответа -- JSON:
{
  ...
  "corrections": [{"field": "d2_voltage", "issue": "Номер страницы некорректен", "corrected_page": 14}],
...
Выполни все 5 задач: полнота, конфликты, косвенные параметры, проверка ссылок,
...
| `confidence == "low"` | -> `warnings` + `present` |
| `"конфликт" in status` | -> `warnings` + `present` |
...
**Параметры окна:**
- Заголовок: `"Normocontrol - Анализ паспортов оборудования"`
...
1. Проверяет наличие файлов и API-ключа
...
try:
    ...
    # Записать error_log.txt (Python path, argv, sys.path, traceback)
    # Открыть error_log.txt через os.startfile()
```

### 12.3. Normocontrol.bat
...
if %errorlevel% equ 0 (
    ...
    echo   ОШИБКА при установке. Запустите от имени администратора.
)
echo.
pause
```
...
2. `Normocontrol.bat` передаёт путь `%APPDATA%\...` как аргумент (Windows раскрывает переменную корректно)
...
**Решение:** `_deduplicate_overlaps(values, overlap=2)` в `resolve_aggregated()` — если два значения из одного файла и их страницы близки (разница ≤ overlap), оставляется первое (из более раннего чанка).
...
При конфликте: сортировка по `(приоритет_типа, приоритет_уверенности)`, берётся первый.
