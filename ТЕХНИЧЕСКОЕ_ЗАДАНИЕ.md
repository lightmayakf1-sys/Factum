...
13. ТОЧНОСТЬ СИМВОЛОВ: При чтении PDF/изображений будь особенно внимателен к:
    - Путанице цифр и букв: 0↔О, 1↔l↔I, П↔11, Б↔6, В↔8, З↔3
    - Индексам и обозначениям: если индекс модели содержит подозрительную
      кириллическую букву среди цифр — это вероятно ошибка распознавания.
      Установи confidence="low" и укажи в note: "возможна ошибка OCR".
...
D.1. Суммарная установленная мощность P_уст (кВт) — сумма ВСЕХ двигателей/приводов. Если перечислены отдельные приводы — укажи каждый и рассчитай Σ. Отдельно P_потр если указана.
...
Выполни все 6 задач: полнота, конфликты, косвенные параметры, проверка ссылок,
логическая непротиворечивость, полнота значений.
